...
// Cores clássicas de 15 bits, meu amigo
#define COLOR_DARKEST   RGB15(1, 7, 1)       // Preto / Linhas de grade
#define COLOR_DARK      RGB15(6, 12, 6)      // Fundo geral (Verde escuro)
#define COLOR_LIGHT     RGB15(17, 21, 1)     // Fundo da caixa (Verde claro)
#define COLOR_LIGHTEST  RGB15(19, 23, 1)     // Texto claro / branco
...
// Constantes para memória SRAM e Segurança, meu amigo
#define SRAM_BASE ((volatile uint8_t*)0x0E000000)
#define SAVE_MAGIC 0x4742 // Assinatura "Retro" para identificar um salvamento válido
#define XOR_KEY 0xA5      // Chave básica para ofuscação

// Matriz de fonte 8x8 comprimida (caracteres ASCII 32 a 127)
const uint8_t font8x8[96][8] = {
    ...
    {0x38, 0x66, 0x76, 0x7e, 0x7e, 0x6e, 0x66, 0x66}, // Caractere modificado nj
    {0x3b, 0x6e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
};

volatile uint16_t* videoBuffer = (volatile uint16_t*)VRAM;

// --- FUNÇÕES DE SEGURANÇA E ARMAZENAMENTO ---

void save_passwords(char passwords[10][11]) {
    volatile uint8_t* sram = SRAM_BASE;
    
    // Escreve a assinatura mágica para validar o salvamento
...
int main() {
    ...
    int save_timer = 0;
    int action_msg = 0; // 0 = Nada, 1 = Salvo, 2 = Apagado
    int l_r_held = 0;   // Controla para que o apagamento ocorra apenas uma vez por pressão
    
    const char* slots[] = {
        "(Slot 1 de 10)", "(Slot 2 de 10)", "(Slot 3 de 10)", "(Slot 4 de 10)",
        "(Slot 5 de 10)", "(Slot 6 de 10)", "(Slot 7 de 10)", "(Slot 8 de 10)",
        "(Slot 9 de 10)", "(Slot 10 de 10)"
    };
    ...
    while (1) {
        ...
        // --- APAGAMENTO TOTAL (L + R Simultaneamente) ---
        if ((keys & KEY_L) && (keys & KEY_R)) {
            if (!l_r_held) {
                // Apaga dados na memória RAM
                for (int i = 0; i < 10; i++) {
                    for (int j = 0; j < 10; j++) {
                        passwords[i][j] = ' ';
                    }
                ...
                // Salva cofre vazio na SRAM
                save_passwords(passwords); 
                
                save_timer = 60;
                action_msg = 2; // Mensagem especial de apagamento
                needs_redraw = 1;
                l_r_held = 1;
            }
            // Mascara L e R para evitar a mudança de slot enquanto segurados
            keys_down &= ~(KEY_L | KEY_R);
        } else {
            l_r_held = 0; // Restaura quando pelo menos um for solto
        }
        ...
        if (keys_down & KEY_START) {
            save_passwords(passwords); 
            save_timer = 60;
            action_msg = 1; // Mensagem de salvamento
            needs_redraw = 1;
        }

        // --- SISTEMA DE ESCRITA D-PAD (Apenas se a senha for visível) ---
        if (show_password) {
        ...
        if (needs_redraw) {
            ...
            // Lógica de mensagens na caixa principal
            if (save_timer > 0) {
                if (action_msg == 1) {
                    drawString(50, 25, "Senha Salva!", COLOR_DARKEST, COLOR_LIGHT, 0, 1);
                } else if (action_msg == 2) {
                    drawString(50, 25, "Cofre Apagado!", COLOR_DARKEST, COLOR_LIGHT, 0, 1);
                }
            } else {
                drawString(28, 25, "Insira sua senha", COLOR_DARKEST, COLOR_LIGHT, 0, 1);
            }

            // Desenha a senha e os cursores
            ...
            // Menu de controles
            drawString(5,  98, "Controles:", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 110, "D-Pad: Move/Muda Letra", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 122, "Select: Mostra/Oculta", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 134, "Start : Salva a senha", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 146, "L / R : Muda o Slot", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
...
// Espero que você aproveite este projeto retrô que fiz com muita diversão.
