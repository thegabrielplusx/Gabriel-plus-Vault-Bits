...
// Klasicke 15bitove barvy, muj priteli
#define COLOR_DARKEST   RGB15(1, 7, 1)       // Cerna / Cary mrizky
#define COLOR_DARK      RGB15(6, 12, 6)      // Celkove pozadi (Tmave zelena)
#define COLOR_LIGHT     RGB15(17, 21, 1)     // Pozadi ramecku (Svetle zelena)
#define COLOR_LIGHTEST  RGB15(19, 23, 1)     // Svetly text / bila
...
// Konstanty pro pamet SRAM a bezpecnost, muj priteli
#define SRAM_BASE ((volatile uint8_t*)0x0E000000)
#define SAVE_MAGIC 0x4742 // Podpis "Retro" k identifikaci platneho ulozeni
#define XOR_KEY 0xA5      // Zakladni klic pro ofuskaci

// Matice komprimovaneho pisma 8x8 (znaky ASCII 32 az 127)
...
int main() {
    ...
    int save_timer = 0;
    int action_msg = 0; // 0 = Nic, 1 = Ulozeno, 2 = Vymazano
    int l_r_held = 0;   // Zajistuje, aby k vymazani doslo pouze jednou pri stisknuti
    
    const char* slots[] = {
        "(Slot 1 z 10)", "(Slot 2 z 10)", "(Slot 3 z 10)", "(Slot 4 z 10)",
        "(Slot 5 z 10)", "(Slot 6 z 10)", "(Slot 7 z 10)", "(Slot 8 z 10)",
        "(Slot 9 z 10)", "(Slot 10 z 10)"
    };
    ...
    while (1) {
        ...
        // --- CELKOVE VYMAZANI (L + R soucasne) ---
        if ((keys & KEY_L) && (keys & KEY_R)) {
            if (!l_r_held) {
                // Vymaze data z pameti RAM
                for (int i = 0; i < 10; i++) {
                    for (int j = 0; j < 10; j++) {
                        passwords[i][j] = ' ';
                    }
                ...
                // Ulozi prazdny trezor do SRAM
                save_passwords(passwords); 
                
                save_timer = 60;
                action_msg = 2; // Specialni zprava o vymazani
                needs_redraw = 1;
                l_r_held = 1;
            }
            // Maskuje L a R, aby se zabranilo zmene slotu, kdyz jsou drzeny
            keys_down &= ~(KEY_L | KEY_R);
        } else {
            l_r_held = 0; // Obnovi se, kdyz je alespon jedno uvolneno
        }
        ...
        if (keys_down & KEY_START) {
            save_passwords(passwords); 
            save_timer = 60;
            action_msg = 1; // Zprava o ulozeni
            needs_redraw = 1;
        }

        // --- SYSTEM PSANI D-PAD (Pouze pokud je heslo viditelne) ---
        if (show_password) {
        ...
        if (needs_redraw) {
            ...
            // Logika zprav v hlavnim ramecku
            if (save_timer > 0) {
                if (action_msg == 1) {
                    drawString(50, 25, "Heslo ulozeno!", COLOR_DARKEST, COLOR_LIGHT, 0, 1);
                } else if (action_msg == 2) {
                    drawString(50, 25, "Trezor smazan!", COLOR_DARKEST, COLOR_LIGHT, 0, 1);
                }
            } else {
                drawString(28, 25, "Zadejte heslo", COLOR_DARKEST, COLOR_LIGHT, 0, 1);
            }

            // Vykresleni hesla a kurzoru
            ...
            // Nabidka ovladani
            drawString(5,  98, "Ovladani:", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 110, "D-Pad: Pohyb/Zmena pism.", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 122, "Select: Ukazat/Skryt", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 134, "Start : Ulozit heslo", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
            drawString(5, 146, "L / R : Zmena slotu", COLOR_LIGHTEST, COLOR_DARK, 1, 1);
...
// Doufam, ze si tento retro projekt, ktery jsem s radosti vytvoril, uzijete.
